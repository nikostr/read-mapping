...
def get_se_illumina_fastq(wildcards):
    ...
    fastqs = samples.loc[
        (wildcards.sample, "illumina", wildcards.unit), ["fq1", "fq2"]
    ].dropna()
    if len(fastqs) == 2:
        raise
...
def get_collect_bams_input(wildcards):
    tmp_df = samples.reset_index(drop=True).assign(
        n_units_per_sample=lambda x: (
            x.groupby("sample_id").sample_id.transform("count")
        )
    )
    targets = {}
    for dt in ["illumina", "ont", "pacbio"]:
        ...
        for cond in ["==1", ">1"]:
            if cond == ">1":
                dt = f"{dt}/merged"
            targets[dt] = (
                dt_df.query(f"n_units_per_sample{cond}")
                .sample_id.apply(lambda x: f"results/mapped/{dt}/{x}.bam")
                .to_list()
            )
...
def get_multiqc_input_from_samtools_stats(wildcards):
    ...
    return [
        re.sub(r"^results/mapped/(.*)\.bam$", r"results/qc/samtools_stats/\1.txt", s)
        for s in bams
    ]
...
def get_multiqc_input_from_deeptools_plotcoverage(wildcards):
    ...
    return [
        re.sub(
            r"^results/mapped/(.*)\.bam$",
            r"results/qc/deeptools_plotcoverage/coverage/\1.raw",
            s,
        )
        for s in bams
    ]
...
def get_bamcoverages(wildcards):
    bams = get_collect_bams_input(wildcards)
    return [
        re.sub(r"^results/mapped/(.*)\.bam$", r"results/mapped/bigwig/\1.bw", s)
        for s in bams
    ]
...
def get_fastqc_input(wildcards):
    ...
    return tmp_df.loc[
        (wildcards.sample, wildcards.unit), ["fq" + wildcards.read]
    ].to_list()
...
def get_multiqc_input_from_fastqc(wildcards):
    tmp_df = (
        ...
        .dropna()
        .reset_index()[["sample_id", "unit", "read"]]
    )

    return [
        f"results/qc/fastqc/{s.sample_id}-{s.unit}-R{s.read}_fastqc.zip"
        for s in tmp_df.itertuples()
    ]
